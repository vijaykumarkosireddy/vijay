  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-20 md:px-12">
      <div className="mb-24 flex flex-col items-center text-center space-y-6 animate-reveal">
        <h1 className="text-5xl font-bold tracking-tighter md:text-7xl title-reveal">{SITE_CONTENT.SCHOOL_NAME}</h1>
        <p className="max-w-3xl text-lg text-foreground/60 font-light">
          Dedicated to the preservation and teaching of Carnatic Vocal music.
          Discover performances and educational sessions led by {SITE_CONTENT.NAME}.
        </p>
      </div>
      ...
      <div className="grid gap-12 md:grid-cols-2">
        {musicItems.length > 0 ? (
              ...
              <div className="flex items-start justify-between px-6">
                <div className="space-y-2">
                  <div className="flex gap-3 items-center">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-gold">{item.platform}</span>
                  </div>
                  <h3 className="text-2xl font-bold tracking-tighter italic">{item.title}</h3>
                </div>
                {item.instagramUrl && (
                  <a href={item.instagramUrl} target="_blank" rel="noopener noreferrer" className="h-12 w-12 rounded-full glass flex items-center justify-center hover:bg-primary transition-all group/icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover/icon:text-primary-foreground"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"></line></svg>
                  </a>
                )}
              </div>
            </div>
          ...
          <div className="col-span-full py-40 text-center space-y-4">
            ...
            <p className="text-sm font-bold tracking-widest text-foreground/20 uppercase">Traditional recitals arriving soon</p>
          </div>
        )}
      </div>
    </div>
