    ...
    <div className="mx-auto max-w-7xl px-6 py-24 md:py-40">
      <div className="mb-32 flex flex-col items-center text-center space-y-10 animate-reveal">
        <div className="inline-block px-4 py-1 rounded-full border border-gold/20 glass">
          <span className="text-[10px] font-black uppercase tracking-[0.5em] text-gold">Public Appreciation Archive</span>
        </div>
        <h1 className="text-6xl md:text-9xl font-black tracking-tighter italic title-reveal leading-[0.8]">Voices of <br /> Gratitude.</h1>
        <p className="max-w-2xl text-xl text-white/40 font-light leading-relaxed italic">
          A full archive of feedback from the global community of Sree Saraswathi Sangeetha Vidhayalam.
        </p>
      </div>
            ...
            <div key={t._id.toString()} className="group relative flex flex-col justify-between animate-reveal-slow p-10 rounded-[2.5rem] glass border-white/5 hover:border-gold/20 transition-all bg-white/[0.01]">
              ...
              <div className="mt-12 flex items-center gap-5 pt-8 border-t border-white/5">
                <div className="h-14 w-14 rounded-2xl overflow-hidden border border-white/10 relative bg-black/40 shadow-xl group-hover:border-gold/30 transition-all">
                  <Image src={t.imageUrl || "/vijay.png"} alt={t.name} fill className="object-cover" />
                </div>
                <div>
                  <h4 className="font-black text-sm uppercase tracking-widest text-gold italic">{t.name}</h4>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 mt-1">{t.role}</p>
                </div>
              </div>
            </div>
          ))
