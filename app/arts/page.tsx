  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-20 md:px-12">
      <div className="mb-24 flex flex-col items-center text-center space-y-6 animate-reveal">
        <h1 className="text-5xl font-bold tracking-tighter md:text-7xl title-reveal">Fine Art Portfolio</h1>
        <p className="max-w-2xl text-lg text-foreground/60 font-light">
          Experience the depth of graphite realism. Each piece is a testament to the discipline of observation and the soul of art.
        </p>
      </div>
      ...
      <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-3">
        {artItems.length > 0 ? (
          artItems.map((item: any) => (
            <div key={item._id.toString()} className="group flex flex-col gap-6 animate-reveal-slow">
              ...
              <div className="px-4 space-y-1">
                ...
                <span className="text-xs font-bold uppercase tracking-widest text-gold opacity-60">Graphite on Paper</span>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-full py-40 text-center space-y-4">
            ...
            <p className="text-sm font-bold tracking-widest text-foreground/20 uppercase">The gallery is currently being curated</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-40 grid gap-16 lg:grid-cols-2 items-center glass p-12 rounded-[3rem] border-accent/5">
        <div className="relative h-[500px] overflow-hidden rounded-[2.5rem]">
          <Image src={SITE_CONTENT.BIO.SECONDARY_IMAGE} alt="Vijay Artist" fill className="object-cover" />
        </div>
        <div className="space-y-8">
          <h2 className="text-4xl font-bold tracking-tighter italic gold-glow inline-block px-4 py-2 bg-primary/10 rounded-xl">The Artist Behind the Pencil</h2>
