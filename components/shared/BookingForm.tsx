...
export default function BookingForm() {
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", interest: "Vocal Music", message: "" });
  ...
  return (
    <div className="glass p-8 md:p-12 rounded-[2.5rem] border-primary/10 gold-glow overflow-hidden relative">
      <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 opacity-[0.03] text-8xl font-black italic gold-glow pointer-events-none">BOOK</div>

      <div className="relative z-10 space-y-8">
        <div className="space-y-2">
          <h3 className="text-3xl font-black tracking-tighter italic text-gold uppercase">Get In Touch</h3>
          <p className="text-sm text-foreground/50 font-light">Music Training • Art Commissions • Consultations</p>
        </div>
        ...
        {status === "success" ? (
          <div className="py-12 text-center space-y-4 animate-scale-in">
            <div className="h-16 w-16 bg-green-500/10 rounded-full flex items-center justify-center mx-auto">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>
            </div>
            <p className="font-bold text-gold uppercase tracking-widest text-xs">Request Sent Successfully</p>
            <button onClick={() => setStatus("idle")} className="text-xs underline opacity-40 hover:opacity-100">Send another</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              >
                <option value="Vocal Music" className="bg-black text-white">Vocal Music</option>
                <option value="Pencil Sketching" className="bg-black text-white">Pencil Sketching</option>
                <option value="Art Commission" className="bg-black text-white">Art Commission</option>
